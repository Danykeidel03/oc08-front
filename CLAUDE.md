...
1. **`export const metadata: Metadata`** (o `generateMetadata` si depende de datos dinámicos), con:
   ...
   - `alternates: { canonical: "<ruta relativa de esa page>" }` (ej. `"/tienda"`, `` `/tienda/${slug}` ``) — sin esto Google puede marcar la página como "Duplicada: sin versión canónica" ante cualquier variante de URL (http vs https, con/sin query params, etc.).
